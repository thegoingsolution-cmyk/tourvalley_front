...
interface ConsentModalMobileProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => void;
  pdfPath?: string;
...
export default function ConsentModalMobile({
  isOpen,
  onClose,
  onConfirm,
  pdfPath,
  insuranceType = '국내여행보험',
}: ConsentModalMobileProps) {
  ...
  return (
    <div 
      className="consent-modal-mobile-overlay" 
      onClick={handleOverlayClick}
    >
      <div 
        className={`consent-modal-mobile-content ${isVisible ? 'visible' : ''}`}
        onClick={(e) => e.stopPropagation()}
      >
        {/* 헤더 - 드래그 핸들 */}
        <div className="consent-modal-mobile-header">
          <div className="consent-modal-mobile-handle"></div>
          <h2 className="consent-modal-mobile-title">
            보험가입을 위한 개인(신용)정보 수집,<br />
            이용, 조회제공 동의서
          </h2>
          <button
            className="consent-modal-mobile-close-btn"
            onClick={onClose}
          >
            ×
          </button>
        ...
        {/* 바디 */}
        <div className="consent-modal-mobile-body">
                ...
                <div className="in_wrap pb5">
                  <ul className="tourG_agree">
                    <li className="tourG_cir tourG_chk">
                      ...
                      />
                      <label htmlFor="all_agree_mobile">
