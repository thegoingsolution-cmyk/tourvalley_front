...
/** 이동 구간 요약 텍스트 (복수 장소일 때 event_location 대체용) */
...
export function validateStep(step: number, state: WizardState): { ok: boolean; errors: ErrorMap } {
  ...
  if (step === 2) {
    ...
    const badLoc = state.locType === '단일' ? !state.region.trim() : !(state.routeFrom.trim() && state.routeTo.trim());
    if (badLoc) {
      errors.region = true;
      ok = false;
    }
...
export function buildFormData(state: WizardState, device: Device): FormData {
  ...
  fd.append('ctel_no', state.phone);
  fd.append('tel_no', state.tel || '');
  ...
  const eventLocation = state.locType === '단일' ? state.region : buildMoveSummary(state);
  fd.append('event_location', eventLocation);
  fd.append('route_from', state.locType === '복수' ? state.routeFrom : '');
  fd.append('route_via', state.locType === '복수' ? state.routeVia : '');
  fd.append('route_to', state.locType === '복수' ? state.routeTo : '');
  fd.append('move_note', state.locType === '복수' ? state.moveNote : '');
