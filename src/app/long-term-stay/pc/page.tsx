...
export default function PCLongTermStayPage() {
  ...
  const recalculatePremium = useCallback(async (medicalExpenseValue?: boolean) => {
    ...
    try {
      ...
      if (medicalExpenseValue !== undefined) {
        const availablePlans = await fetchAvailablePlans(insuranceType, age, genderValue, medicalExpense, planOptions);
  ...
  const handleCalculateParticipants = async () => {
    ...
    try {
      ...
      for (const participant of participants) {
        ...
        const currencyPlanValue = travelPurpose === '워킹홀리데이'
  ...
  const handleCalculate = async (overrideCurrencyPlan?: '원화' | '외화') => {
    ...
    try {
      ...
      const availablePlans = await fetchAvailablePlans(String(travelPurpose || ''), Number(age), genderValue, undefined, {
        birth_date: birthDate,
        departure_date: departureDateTime,
      });
      if (availablePlans.length === 0) {
